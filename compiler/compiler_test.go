...
// TestCompilerScope is used to make sure that the instructions emitted in one scope
// should have no effect on the instructions in another scope.
...
func TestFloatArithmetic(t *testing.T) {
	tests := []compilerTestCase{
		...
		{
			input:             "2.5 / 1.5",
			expectedConstants: []interface{}{2.5, 1.5},
			expectedInstructions: []code.Instructions{
				code.Make(code.OpConstant, 0),
				code.Make(code.OpConstant, 1),
				code.Make(code.OpDiv),
	...
	tests := []compilerTestCase{
		{
			...
			expectedInstructions: []code.Instructions{
				...
				// 0010
				code.Make(code.OpNull), // alternative block.
				// 0011
				code.Make(code.OpPop),
				// 0012
				code.Make(code.OpConstant, 1),
...
func TestHashLiterals(t *testing.T) {
	tests := []compilerTestCase{
		...
		{
			input:             "{1.5: 2.5 * 4.0, 5.5: 1.5 + 1.5}",
			expectedConstants: []interface{}{1.5, 2.5, 4.0, 5.5, 1.5, 1.5},
			expectedInstructions: []code.Instructions{
				code.Make(code.OpConstant, 0),
				code.Make(code.OpConstant, 1),
				code.Make(code.OpConstant, 2),
...
func runCompilerTests(t *testing.T, tests []compilerTestCase) {
	// Helper method allows us to remove duplicated logic in test functions
	// by defining test helpers.
	t.Helper()
...
// concatInstructions is used to turn the slice of slices into a flattend slice
// by concatenating the instructions in order to compare the expectedInstructions
...
func testConstants(t *testing.T, expected []interface{}, actual []object.Object) error {
	...
	for i, constant := range expected {
		...
		case float64:
			...
			if err != nil {
				return fmt.Errorf("constant %+v - testFloatObject failed: %s",
					i, err)
			}
