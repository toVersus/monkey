...
// It is used for evaluating arguments of function from left-to-right.
...
// unwrapReturnValue evaluates the function's body if it is an *object.ReturnValue
// in order to stop the evaluation of the last called function's body.
...
// evalIndexExpression checks left operand of the index operator and index itself
// and returns error if its type is not supported in index operator.
func evalIndexExpression(left, index object.Object) object.Object {
	...
	default:
		return newError("index operator not supported for %s: %s", left.Type(), index.Type())
	}
}

// evalArrayIndexExpression checks the given index is out of range and it returns NULL if it exceeds the range.
...
// evalHashLiteral iterates over the node and evaluates keyNode in first.
// It checks if the call to Eval and type assertion about the evaluation result,
// then it evaluates valuNode and adds the newly produced key-value pair to map insetance.
